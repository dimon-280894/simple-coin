...
*Setup instructions for a Gitian build of Gentarium Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gentarium
...
- [Building Gentarium Core](#building-gentarium-core)
...
Clone the git repositories for Gentarium Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://https://github.com/dimon-280894/simple-coin
```
...
in the Gentarium Core repository under 'Fetch and build inputs' to install sources which require
...
To build Gentarium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Gentarium Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gentarium/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://https://github.com/dimon-280894/simple-coin
...
For example:
```bash
URL=https://github.com/crowning-/gentarium.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gentarium=${COMMIT} --url gentarium=${URL} ../gentarium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gentarium=${COMMIT} --url gentarium=${URL} ../gentarium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gentarium=${COMMIT} --url gentarium=${URL} ../gentarium/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gentarium git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gentarium/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gentarium.git
SIGPATH=/some/root/path/gentarium-detached-sigs.git

./bin/gbuild --url gentarium=${BTCPATH},signature=${SIGPATH} ../gentarium/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gentarium-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gentarium-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gentarium-osx-build.assert
```
...
[gentarium/gitian.sigs](https://github.com/dimon-280894/simple-coin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
