...
### Send 1000 GTM to New Addresses

Send exactly 1000 GTM to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 GTM.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 GTM on the remote server and defeats the purpose of a hot/cold setup.*
...
## Update gentarium.conf on server

If you generated a new masternode private key, you will need to update the remote `gentarium.conf` files.
