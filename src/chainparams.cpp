...
static CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
{
    ...
    return CreateGenesisBlock(PSZ_TIMESTAMP, genesisOutputScript, nTime, nNonce, nBits, nVersion, genesisReward);
}
...
public:
    CMainParams() {
        ...
        consensus.nMasternodePaymentsStartBlock = 3600; // ~5 days.Not true, but it's ok as long as it's less then nMasternodePaymentsIncreaseBlock
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Gentarium: 1 day
        consensus.nPowTargetSpacing = 2 * 60; // Gentarium: 2 minutes
        ...
        // Gentarium script addresses start with '7'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,10);
        // Gentarium private keys start with '7' or 'G'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,198);
        // Gentarium BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x04)(0x88)(0xB2)(0x1E).convert_to_container<std::vector<unsigned char> >();
        // Gentarium BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = boost::assign::list_of(0x04)(0x88)(0xAD)(0xE4).convert_to_container<std::vector<unsigned char> >();

        // Gentarium BIP44 coin type is '5'
        nExtCoinType = 5;
        ...
        checkpointData = (CCheckpointData) {
            ...
            ,
            1533609063, // * UNIX timestamp of last checkpoint block
            12864,          // * total number of transactions between genesis and last checkpoint
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S(TEST_GENESIS); // 37900
        ...
        genesis = CreateGenesisBlock(TEST_TIMESTAMP, TEST_NONCE, TEST_BITS, TEST_VERSION, SINGLE_BLOCK_REWARD * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S(TEST_GENESIS));
        assert(genesis.hashMerkleRoot == uint256S(HASH_MERKLE_ROOT));

        vFixedSeeds.clear();
        vSeeds.clear();

        // Testnet Gentarium addresses start with 'G'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 38);
        // Testnet Gentarium script addresses start with '5'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 10);
        // Testnet private keys start with 'G'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1, 198);
        // Testnet Gentarium BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x04)(0x35)(0x87)(0xCF).convert_to_container<std::vector<unsigned char> >();
        // Testnet Gentarium BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = boost::assign::list_of(0x04)(0x35)(0x83)(0x94).convert_to_container<std::vector<unsigned char> >();

        // Testnet Gentarium BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
        ...
        strSporkPubKey = TEST_SPORK_PUB_KEY;

        checkpointData = (CCheckpointData) {
            boost::assign::map_list_of
            (    0, uint256S(TEST_GENESIS)),

            TEST_TIMESTAMP, // * UNIX timestamp of last checkpoint block
...
public:
    CRegTestParams() {
        ...
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S(REGTEST_GENESIS));
        assert(genesis.hashMerkleRoot == uint256S(HASH_MERKLE_ROOT));
        ...
        checkpointData = (CCheckpointData){
            boost::assign::map_list_of
            ( 0, uint256S(REGTEST_GENESIS)),
            0,
            0,
            0
        };
        // Regtest Gentarium addresses start with 'G'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,38);
        // Regtest Gentarium script addresses start with '5'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,10);
        // Regtest private keys start with'G'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,198);
        // Regtest Gentarium BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x04)(0x35)(0x87)(0xCF).convert_to_container<std::vector<unsigned char> >();
        // Regtest Gentarium BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = boost::assign::list_of(0x04)(0x35)(0x83)(0x94).convert_to_container<std::vector<unsigned char> >();

        // Regtest Gentarium BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
